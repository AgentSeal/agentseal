...
export function saveCredentials(apiUrl: string, apiKey: string, path: string = join(CONFIG_DIR, "config.json")): void {
  const dir = dirname(path);
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true, mode: 0o700 });
  const cfg = existsSync(path) ? JSON.parse(readFileSync(path, "utf-8")) : {};
  cfg["dashboard-url"] = apiUrl;
  cfg["dashboard-key"] = apiKey;
  writeFileSync(path, JSON.stringify(cfg, null, 2), { mode: 0o600 });
  chmodSync(path, 0o600);
}

export function loadCredentials(path: string = join(CONFIG_DIR, "config.json")): Credentials | null {
